...
// NUEVA FUNCIÓN: Obtener lista única de deportes activos
export const getDeportesUnicos = async (): Promise<string[]> => {
    const torneos = await getTorneos();
    // Extraemos deportes, convertimos a mayúsculas, eliminamos duplicados y ordenamos
    const deportes = Array.from(new Set(torneos.map(t => (t.deporte || 'GENERAL').toUpperCase())));
    return ['TODOS', ...deportes.sort()];
};