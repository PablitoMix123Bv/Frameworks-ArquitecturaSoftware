...
import { getMiEquipo, inscribirEquipoEnTorneo, crearEquipo } from '../services/inscripcionesService';
...
const InscripcionesPage: React.FC = () => {
    ...
    // CORRECCIÓN: Verificamos si el array de roles incluye 'capitan'
    const esCapitan = isLoggedIn && user?.roles.includes('capitan');
    ...
    useEffect(() => {
        const initData = async () => {
            ...
            // Verificamos array de roles también aquí
            if (isLoggedIn && user && user.roles.includes('capitan')) {
                const equipo = await getMiEquipo(user.id);
                // Forzamos el tipo si es necesario o dejamos que undefined fluya
